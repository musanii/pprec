...
<div class="flex items-center justify-between mb-5">
    <div>
        <h1 class="text-2xl font-semibold text-slate-900">Add Term</h1>
        <p class="text-sm text-slate-500 mt-1">Create a term under an academic year</p>
    </div>
...
<form method="POST" action="{{ route('admin.terms.store') }}" class="max-w-3xl space-y-6">
    @csrf
        ...
        <div class="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="md:col-span-2">
                ...
                <select name="academic_year_id"
                    ...
                    @foreach($years as $y)
                        <option value="{{ $y->id }}" @selected((string)old('academic_year_id') === (string)$y->id)>
            ...
            <div class="md:col-span-2">
                ...
                <input name="name" value="{{ old('name') }}" placeholder="e.g. Term 1"
            ...
            <div class="md:col-span-2">
                <label class="inline-flex items-center gap-2 text-sm text-slate-700">
                    ...
                    Set as active immediately (also activates its year)
                </label>
            </div>
        </div>
    </div>
