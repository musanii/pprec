...
<div class="flex items-center justify-between mb-5">
    <div>
        ...
        <p class="text-sm text-slate-500 mt-1">Manage term periods and activation</p>
    </div>
    ...
    <div class="p-4 sm:p-5 border-b border-slate-100">
        <form method="GET" action="{{ route('admin.terms.index') }}"
            ...
            <div class="md:col-span-4">
                ...
                <input name="q" value="{{ request('q') }}" placeholder="Term name"
            ...
            <div class="md:col-span-4">
                ...
                <select name="academic_year_id"
                    ...
                    @foreach($years as $y)
                        <option value="{{ $y->id }}" @selected((int)request('academic_year_id') === $y->id)>
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full">
            ...
            <tbody class="divide-y">
                @forelse($terms as $term)
                    <tr class="hover:bg-slate-50 transition">
                        <td class="px-6 py-4">
                            ...
                            <div class="text-xs text-slate-500">ID: {{ $term->id }}</div>
                        </td>

                        <td class="px-6 py-4 text-slate-700">{{ $term->academicYear?->name ?? '—' }}</td>
                        <td class="px-6 py-4 text-slate-700">{{ \Carbon\Carbon::parse($term->start_date)->format('d M Y') }}</td>
                        <td class="px-6 py-4 text-slate-700">{{ \Carbon\Carbon::parse($term->end_date)->format('d M Y') }}</td>

                        <td class="px-6 py-4">
                            <span class="inline-flex items-center px-2.5 py-1 text-xs border rounded-full
                                {{ $term->is_active ? 'bg-green-50 text-green-700 border-green-200' : 'bg-slate-50 text-slate-700 border-slate-200' }}">
                                    ...
                                    <div x-show="open" x-transition.opacity @click.outside="open=false" x-cloak
                                        ...
                                        <div class="w-48 rounded-xl border border-slate-200 bg-white shadow-lg overflow-hidden">
                                            ...
                                            <form method="POST" action="{{ route('admin.terms.activate', $term) }}">
                                                ...
                                                <button type="submit"
                                                        class="w-full text-left px-4 py-2.5 text-sm hover:bg-slate-50 {{ $term->is_active ? 'text-slate-400 cursor-not-allowed' : 'text-slate-700' }}"
                                                        {{ $term->is_active ? 'disabled' : '' }}>
                                                    Set Active
                                                </button>
                                            </form>
                                        </div>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center text-slate-500">No terms found.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
