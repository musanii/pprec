...
<div class="flex items-center justify-between mb-5">
    <div>
        <h1 class="text-2xl font-semibold text-slate-900">Edit Class</h1>
        <p class="text-sm text-slate-500 mt-1">Edit a class level</p>
    </div>
...
<form method="POST" action="{{ route('admin.classes.update', $class) }}"  class="max-w-3xl space-y-6">
    @csrf
    @method('PUT')
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div class="p-5 border-b border-slate-100">
            ...
            <div class="text-xs text-slate-500 mt-1">Name and ordering level.</div>
        </div>
        ...
        <div class="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="number" name="level" value="{{ old('level', $class->level) }}"
                       class="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm bg-white
                              focus:outline-none focus:ring-2 focus:ring-primary/20
                              @error('level') border-red-300 ring-2 ring-red-100 @else border-slate-200 @enderror"
            ...
            <div class="flex items-end">
                <label class="inline-flex items-center gap-2 text-sm text-slate-700">
                    <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $class->is_active))
