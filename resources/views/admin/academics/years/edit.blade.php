...
<div class="flex items-center justify-between mb-5">
    <div>
        <h1 class="text-2xl font-semibold text-slate-900">Edit Academic Year</h1>
        <p class="text-sm text-slate-500 mt-1">Update year details and activation</p>
    </div>
...
<form method="POST" action="{{ route('admin.academic-years.update', $year) }}" class="max-w-3xl space-y-6">
    @csrf
    @method('PUT')
    ...
    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        ...
        <div class="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="date" name="start_date" value="{{ old('start_date', \Carbon\Carbon::parse($year->start_date)->format('Y-m-d')) }}"
                       class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm
            ...
            <div>
                ...
                <input type="date" name="end_date" value="{{ old('end_date', \Carbon\Carbon::parse($year->end_date)->format('Y-m-d')) }}"
                       class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm
            ...
            <div class="md:col-span-2">
                ...
                <div class="text-xs text-slate-500 mt-1">Only one year can be active at a time.</div>
            </div>
        </div>
    </div>
