...
<div class="flex items-center justify-between mb-5">
    <div>
        <h1 class="text-2xl font-semibold text-slate-900">Add Subject</h1>
        <p class="text-sm text-slate-500 mt-1">Create a new curriculum subject</p>
    </div>
...
<form method="POST" action="{{ route('admin.subjects.store') }}" class="max-w-3xl space-y-6">
    @csrf

    <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div class="p-5 border-b border-slate-100">
            <div class="text-sm font-semibold text-slate-900">Subject Details</div>
            <div class="text-xs text-slate-500 mt-1">Basic subject configuration.</div>
        </div>

        <div class="p-5 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="md:col-span-2">
                <label class="text-xs font-medium text-slate-600">Subject Name</label>
                <input
                    name="name"
                    value="{{ old('name') }}"
            ...
            <div>
                ...
                <input
                    ...
                    value="{{ old('code') }}"
                    placeholder="MATH"
                    class="mt-1 w-full rounded-xl border px-3 py-2.5 text-sm uppercase
                           focus:outline-none focus:ring-2 focus:ring-primary/20
                           @error('code') border-red-300 ring-2 ring-red-100 @else border-slate-200 @enderror"
                    required
                >
